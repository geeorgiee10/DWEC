    ...
    if (document.cookie === "") {
        ...
        btnCrear.addEventListener("click", () => {
            if (usuarios(nombUsu.value)) {
                ...
                // Aquí no añadimos expiración a la cookie de configuración, para que sea de sesión
                let config = {
                    colorFondo: colorFondo.value,
                    colorTexto: colorTexto.value,
                    tamanoLetra: tamanoLetra.value
                };
                document.cookie = `configuracion=${encodeURIComponent(JSON.stringify(config))}; path=/`;
