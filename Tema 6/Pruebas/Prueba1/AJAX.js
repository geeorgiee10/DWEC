...
/*function peticionAJAX() {
    ...
    xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
        ...
        datos.players.forEach(element => {
            let li = document.createElement('li');
            li.innerHTML = element.name;

                datos.players.forEach(item =>{
                    if(item.name == element.name){
                        let subLista = document.createElement("ul");
                        let subli = document.createElement("li");
                        let subli2 = document.createElement("li");
                        let subli3 = document.createElement("li");
                        let subli4 = document.createElement("li");
                        subli.innerHTML = item.position;
                        subli2.innerHTML = item.nationality;
                        subli3.innerHTML = item.goals;
                        subli4.innerHTML = "Tiene Champions: " + item.champions;

                        datos.players.forEach(el =>{
                            if(el.name == item.name){
                                let subSubLista = document.createElement("ul");
                                let subSubli = document.createElement("li");

                                subSubli.innerHTML = "Número de Champions: " + el.numChampions;
                                subSubLista.appendChild(subSubli);

                                subli4.appendChild(subSubLista);
                            }
                        })

                        subLista.appendChild(subli);
                        subLista.appendChild(subli2);
                        subLista.appendChild(subli3);
                        subLista.appendChild(subli4);
    ...
    xhttp2.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            titulo.innerHTML = this.responseText;
        }
    }
    ...
    xhttp2.open("GET", "titulo.txt", true);
    xhttp.send();
    xhttp2.send();
}*/
